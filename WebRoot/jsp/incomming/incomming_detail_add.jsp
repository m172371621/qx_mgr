...
<div id="mright">
	<div id="mr_cnt">
		...
		if(stock_type==1){ 
		%>
					<div class="mrw_title">添加入库单明细</div>
		<%}else if(stock_type==2){ %>
		            <div class="mrw_title">添加调入单明细</div>
		<%} %>
...
<script type="text/javascript">
	...
	function addGoods(data){
	   var str='<tr>'
			   ...
			   +'<td>'+data.barcode+'</td>'	
			   +'<td>'+data.price+'</td>	'
			   +'<td><input type="text" style="width:180px" name="change_count" value="1" onkeyup="checkCount(this)"/></td>	'
	...
	function postData(){
	   ...
	   if(products.length>0){
	      $.ajax({
             type: 'POST',
             url: "${pageContext.request.contextPath }/incommmingOrder.do?method=add_incomming_detail",
             data:{products:JSON.stringify(products),stock_type:stock_type},
             success: function(data){
                   var dataObj = eval("("+data+")");
                   if(dataObj.result_code=="0"){
                       location.href="${pageContext.request.contextPath }/incommmingOrder.do?method=list_incomming_head&stock_type="+stock_type;
                   }
             }});
	   }
	}
