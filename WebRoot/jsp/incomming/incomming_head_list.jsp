...
<div id="mright">
	<div id="mr_cnt">
		...
		if(stock_type==1){ 
		%>
					<div class="mrw_title">入库单列表</div>
		<%}else if(stock_type==2){ %>
		            <div class="mrw_title">调入单列表</div>
		<%} %>
					<div class="sform_wrap c">
							...
							<ul>
								...
								<%if(stock_type==1){%>
					<li><a href="${pageContext.request.contextPath }/incommmingOrder.do?method=add_detail_url&stock_type=<%=request.getParameter("stock_type") %>" class="btn btn_r">发起入库单</a></li>
		<%}else if(stock_type==2){ %>
		           <li><a href="${pageContext.request.contextPath }/incommmingOrder.do?method=add_detail_url&stock_type=<%=request.getParameter("stock_type") %>" class="btn btn_r">发起调入单</a></li>
		<%} %>
					...
					<form action="${pageContext.request.contextPath }/incommmingOrder.do?method=list_incomming_head&stock_type=<%=request.getParameter("stock_type") %>" method="post" id="searchForm">
					<div id="order_wrap" class="p20">
						...
						<colgroup><col width="160px"/><col/><col/><col/></colgroup>
							...
							<tr>
								...
								-->
								<td>									
									<a href="${pageContext.request.contextPath }/incommmingOrder.do?method=list_incomming_detail&stockchangeHeaderId=<%= headerbean.getStockchange_header_id() %>" class="btn btn_r">详情</a>									
								</td>
							</tr>
							<% }} %>
						</table>
...
<script type="text/javascript">
		...
		{  
			...
			location.href="${pageContext.request.contextPath }/incommmingOrder.do?method=list_incomming_detail&stockchangeHeaderId="+id;
