					...
					<div class="sform_wrap c">
							...
							%>
								...
								<li>所属服务：<select name="sid1"  id="" class="i_text">
									<%
										List<Role> rolelist = (List<Role>)request.getSession().getAttribute("user_role");
										if(rolelist.get(0).getRole_id() < 3)
										{
									%>
									<option value="0"  >全部</option>
									<%} %>
									<%
									   List<ServiceVo> slist = (List<ServiceVo>)session.getAttribute("user_service");
								...
								<li>订单时间从：<input readonly type="text" class="i_text" name="start_time1" value="<%= JsonUtil.obj2Str(searchBean.getStart_time()) %>"/></li>
						...
						<form action="${pageContext.request.contextPath }/order.do?method=orderPrice" method="post" id="searchForm">
					...
					<div id="order_wrap" class="p20">
						...
						<colgroup><col width="160px"/><col width="180px"/><col width="80px"/><col width="80px"/><col width="80px"/><col width="80px"/><col width="80px"/><col width="160px"/><col width="80px"/></colgroup>
							...
							<tr>
								...
								<td><%= time %></td>
								<td><a  target="_blank" href="${pageContext.request.contextPath }/order.do?method=edit&type=3&viewId=<%= order.getOrder_id() %>" class="btn btn_r">定价</a></td>
							</tr>
							<% }} %>
						</table>
					</div>
